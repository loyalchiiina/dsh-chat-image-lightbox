...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function makeBtn(className, html, title, handler) {
    ...
    function ensureLightbox() {
      ...
      toolbar.appendChild(makeBtn("", "⬇", "下载图片", function () { downloadCurrent(); }));
      toolbar.appendChild(makeBtn("", "✕", "关闭 (Esc)", function () { close(); }));

      prevBtn = makeBtn("dsh-ig-nav dsh-ig-prev dsh-ig-hidden", "‹", "上一张", function () { navigate(-1); });
      nextBtn = makeBtn("dsh-ig-nav dsh-ig-next dsh-ig-hidden", "›", "下一张", function () { navigate(1); });      backdrop.appendChild(counterEl);

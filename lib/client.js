...
window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function isChatImage(img) {
      ...
      // 🔧 v1.7.1 fix: never adopt images rendered by our own toolview row
      //    (display_image results). They live outside any message block, so
      //    adopting them creates a class tug-of-war with React that ends in
      //    the rows being unmounted. See DisplayImageRow for the full note.
      if (img.dataset && img.dataset.dshIgSkip === "1") { lastSkipReason = "toolview图(skip)"; return false; }
      if (img.classList && img.classList.contains("dsh-ig-toolimg")) { lastSkipReason = "toolview图(class)"; return false; }
      ...
      return igReact.createElement("div",
        ...
        urls.map(function (u, idx) {
          return igReact.createElement("img", {
            key: idx,
            // 🔧 v1.7.1 fix: toolview rows are NOT message blocks, so
            //    allBlocks()/syncFoldState can never manage the images they
            //    create. If isChatImage() adopts them anyway, the observer
            //    re-applies the class on every mutation while React's next
            //    render wipes className again — an endless tug-of-war that
            //    ends with React unmounting the whole row (images flash once
            //    and vanish, and scrolling back does not bring them back).
            //    Stamp the image so isChatImage() leaves it alone.
            "data-dsh-ig-skip": "1",
            className: "dsh-ig-toolimg",
            src: u,
            alt: "",
